...
int32_t ImageProcessor::Process(cv::Mat& eye_image, cv::Mat& mat, ImageProcessor::Result& result)
{
    if (!s_facedet_engine || !s_facemesh_engine) {
        PRINT_E("Not initialized\n");
        return -1;
    ...
    */    
    ...
    if (s_facemesh_engine->Process(eye_image, mat, det_result.bbox_list, facemesh_result_list) != FacemeshEngine::kRetOk) {
        return -1;
    }

     
    int s = facemesh_result_list.size();
    for(int i=0;i<s;i++) {
      std::array<std::pair<int32_t,int32_t>,468 >  &arr = facemesh_result_list[i].keypoint_list;
      int s2 = 468;
    }
    ...
    for (const auto& facemesh_result : facemesh_result_list) {
        ...
        for (int32_t i = 0; i < sizeof(kMeshIndices) / sizeof(kMeshIndices[0]); i += 3) {
            ...
            cv::Point p2(facemesh_result.keypoint_list[kMeshIndices[i + 2]].first, facemesh_result.keypoint_list[kMeshIndices[i + 2]].second);
