...
int32_t FacemeshEngine::Initialize(const std::string& work_dir, const int32_t num_threads)
{
    ...
    eye_sticker = new SelfieSticker::EyeSticker();
...
int32_t FacemeshEngine::Process(const cv::Mat& eye_image, const cv::Mat& original_mat, const std::vector<BoundingBox>& bbox_list, std::vector<Result>& result_list)
{
    if (!inference_helper_) {
        PRINT_E("Inference helper is not created\n");
        return kRetErr;
    ...
    for (const auto& bbox : bbox_list) {
        ...
        float scale2_w = static_cast<float>(crop_w) / input_tensor_info.GetWidth();
        float scale2_h = static_cast<float>(crop_h)  / input_tensor_info.GetHeight();
        float crop2_x = crop_x;
        float crop2_y = crop_y;

        /* reference : https://github.com/google/mediapipe/blob/master/docs/solutions/face_mesh.md#output */
        Result result;
        result.score = score_list[0];
        std::vector<Point3_t<float> > result_vec;
        for (size_t i = 0; i < result.keypoint_list.size(); i++) {
            ...
            p.x = static_cast<float>(landmark_list[3 * i + 0] * scale2_w + 0.5f + crop2_x)/original_mat.cols;
            p.y = static_cast<float>(landmark_list[3 * i + 1] * scale2_h + 0.5f + crop2_y)/original_mat.rows;
            p.z = static_cast<float>(landmark_list[3 * i + 2] * scale2_w + 0.5f + crop2_x)/original_mat.cols;

        /*
            std::cout<<"result.keypoint_list[i].first"<<" "<<result.keypoint_list[i].first <<std::endl;
            std::cout<<"p.x"<<" "<<p.x <<std::endl;
            std::cout<<"result.keypoint_list[i].second"<<" "<<result.keypoint_list[i].second <<std::endl;
            std::cout<<"p.y"<<" "<<p.y <<std::endl;
        */
            result_vec.push_back(p);
        }

        cv::Mat src = original_mat.clone(); 
        cv::cvtColor(src, src, cv::COLOR_BGR2RGB);

        eye_sticker->sticker_image = eye_image;
        src = eye_sticker->apply(src, result_vec);

        std::cout << "success" << std::endl;

        // std::ofstream ss("our_output.txt");
        // int s = result_vec.size();
        // for(int i=0;i<s;i++) ss<< result_vec[i].x << " "<< result_vec[i].y << " " << result_vec[i].z << std::endl;
        // ss.close();
